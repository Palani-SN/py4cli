...
class gen_parser:
    ...
    def __init__(self, inps:list=sys.argv):
        ...
        if def_func_name in methods:
            if len(inps) == 2 and (inps[1] in ['-h', '--help']):
                ...
                args_schema = self._inps2schema(def_func_name, inps)
...
#     def _inps2schema(self, def_func_name, cnfs):

#         cnfs_schema = self.__cnfs2schema(def_func_name, pathlib.Path(argv[-1]))
#         return cnfs_schema
