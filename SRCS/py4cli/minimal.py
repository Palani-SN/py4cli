...
class cnf_parser:
    ...
    def __init__(self, argv:list=sys.argv):
        ...
        if def_func_name in methods:
            if len(argv) == 2 and (argv[1] in ['-h', '--help']):
                ...
                func_schema = self.__func2schema(def_func_name)
                args_schema = self.__cnfs2schema(def_func_name, pathlib.Path(argv[-1]))
                args, kwargs = self.__solve_schema(func_schema[def_func_name], args_schema[def_func_name])
                self.returned = self.__func(def_func_name, args, kwargs)
                if func_schema['ret_type'] != inspect._empty and type(self.returned) != func_schema['ret_type']:
                    print(
    ...
    def __cnfs2schema(self, func_name, inp_file):

        if inp_file.name == __main__.__file__:
    ...
    # def __init__(self, cnfs:str = pathlib.Path(sys.argv[1])) -> None:

        # if not cnfs.exists():
        #     raise FileNotFoundError(cnfs)
        
        # if cnfs.name.endswith(('.yml', '.yaml')):
        #     with open(cnfs.absolute(), 'r') as stream:
        #         try:
        #             cnfs2dict = yaml.load(stream, Loader=SafeLoader)
        #         except yaml.YAMLError as err:
        #             print(f'{err} in {cnfs.absolute()}')
        # elif cnfs.name.endswith('.json'):
        #     with open(cnfs.absolute(), 'r') as file:
        #         cnfs2dict = json.load(file)
